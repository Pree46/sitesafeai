...
# aiortc is optional. If it's not installed, we keep the module importable
try:
    ...
    logger.warning("aiortc not installed; WebRTC endpoints will be disabled. To enable, run: pip install aiortc av")
...
if AIORTC_AVAILABLE:
    ...
    @video_webrtc_router.post("/ws/webrtc-offer")
    async def webrtc_offer(offer: dict = None):
        """WebRTC endpoint unavailable because aiortc isn't installed."""
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="aiortc not installed; WebRTC endpoint unavailable")
